...
export default {
  ...
  computed: {
    ...
    // 创建计算属性，获取vuex中的数据，都是store中的getters中的数据
    ...mapGetters([
      'avatar',
      'permission_routes',
      'name'
  ...
  methods: {
    ...
    resolvePath(item) {
      ...
      const getDefaultPath = (item, parent) => {
        ...
        // 存在个问题：如果是外部链接点击跳转后当前页内容还是上一个路由内容
        if (isExternal(item.path)) {
          path = item.path
          return
        }
    ...
    handleSelect(key, keyPath) {
      // 通过遍历store中的permission_routes，找到当前路由对应的菜单项
      const route = this.permission_routes.find(item => item.path === key)
      // 将当前路由对应的菜单项存到store中，用来渲染左侧菜单
      this.$store.commit('permission/SET_CURRENT_ROUTES', route)
      // 设置侧边栏的显示和隐藏
      this.setSidebarHide(route)
    },
